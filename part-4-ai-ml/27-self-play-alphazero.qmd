...
mcts_search <- function(board, n_iterations = 1000, c_explore = sqrt(2)) {
...
```{r}
#| label: fig-mcts-tree
#| fig-cap: "MCTS search from an empty Tic-Tac-Toe board (1000 iterations). Each cell shows visit count and win rate from X's perspective. The center (position 5) receives the most visits, reflecting its strategic importance. Corner moves also receive high visit counts, consistent with known optimal opening play."
#| fig-width: 6
#| fig-height: 6

set.seed(42)
result <- mcts_search(ttt_new(), n_iterations = 1000)
...
p_tree <- ggplot(board_df, aes(x = col, y = row)) +
  ...
  labs(title = "MCTS Visit Counts from Empty Board",
       subtitle = "1000 iterations, X to move",
       x = NULL, y = NULL)
...
for (n_iter in iteration_levels) {
  outcomes <- replicate(n_games, mcts_vs_random(n_iter))
  self_play_results <- bind_rows(self_play_results, tibble(
    iterations = n_iter,
    win_rate = mean(outcomes == 1),
    draw_rate = mean(outcomes == 0)
  ))
}
```
...
#| fig-cap: "MCTS win rate against a random opponent as search budget increases. With only 10 iterations per move, MCTS wins roughly 70% of games. By 200 iterations it wins nearly every game, demonstrating how deeper search translates directly into stronger play."
#| fig-width: 7
#| fig-height: 5
...
set.seed(42)
result_mid <- mcts_search(board_mid, n_iterations = 2000)

cat("\nMCTS analysis (2000 iterations):\n")
