...
author: "Raban Heller"
date-modified: today
abstract: "Tabular Q-learning in matrix games: convergence in coordination games, divergence in zero-sum games, and the rational learning debate."
keywords: ["Q-learning", "matrix games", "convergence", "multi-agent", "rational learning"]
other-links:
  - text: Download Chapter PDF
    href: ../downloads/q-learning-in-games.pdf
    icon: file-pdf
---
...
#| fig-cap: "Q-value evolution in a coordination game. Both agents converge to action A (the payoff-dominant Nash equilibrium), with Q-values stabilizing near the equilibrium payoff of 2."
#| fig-width: 7
#| fig-height: 5
...
```{r}
#| label: fig-q-cycling
#| fig-cap: "Q-value evolution in Matching Pennies (zero-sum). Instead of converging, Q-values oscillate as each agent's adaptation shifts the other's optimal play. The mixed NE at (0.5, 0.5) is never stably reached."
#| fig-width: 7
#| fig-height: 5
...
- **Multi-agent RL** (@sec-multi-agent-rl) covers advanced approaches: MADDPG, PSRO, and the challenge of non-stationarity at scale.
- **Reinforcement learning foundations** (@sec-reinforcement-learning) provides the single-agent background that this chapter builds on.
