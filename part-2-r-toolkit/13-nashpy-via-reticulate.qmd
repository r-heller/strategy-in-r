...
author: "Raban Heller"
date-modified: today
abstract: "Accessing Python's nashpy library from R via reticulate for Nash equilibrium computation --- conceptual workflow, pure-R support enumeration as an equivalent alternative, and a comparative analysis across canonical games."
keywords: ["nashpy", "reticulate", "Python", "Nash equilibrium", "support enumeration", "mixed strategies"]
other-links:
  - text: Download Chapter PDF
    href: ../downloads/nashpy-via-reticulate.pdf
    icon: file-pdf
---
...
R's ecosystem for non-cooperative game theory is modest. The solvers we built in @sec-nash-equilibrium handle 2x2 games, but many interesting strategic interactions are larger: a 3x3 Rock-Paper-Scissors variant, a 4x4 coordination game, or an asymmetric contest with different numbers of strategies per player. For these cases, we need general-purpose solvers.
...
Several patterns emerge from @fig-equilibria-comparison:
...
- **Nash equilibrium** (@sec-nash-equilibrium) provides the theoretical foundation. Nash's existence theorem guarantees that support enumeration will always find at least one equilibrium for any finite game.
- **Mixed strategies** (@sec-mixed-strategies) develops the indifference principle that the algorithm operationalizes.
- **Custom solvers** (@sec-custom-solvers) extends the toolkit with the Lemke--Howson algorithm and fictitious play, which are more efficient for larger games.
- **The gtree package** (@sec-gtree-package) handles extensive-form games; converting an extensive form to normal form and then applying support enumeration is a standard workflow.
