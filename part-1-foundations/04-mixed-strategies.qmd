...
author: "Raban Heller"
date-modified: today
abstract: "Why rational players sometimes randomize, how the indifference principle pins down mixing probabilities, and how to compute and visualize mixed-strategy Nash equilibria in R."
keywords: ["mixed strategies", "Nash equilibrium", "indifference principle", "support enumeration", "R"]
other-links:
  - text: Download Chapter PDF
    href: ../downloads/mixed-strategies-and-the-indifference-principle.pdf
    icon: file-pdf
---
...
The function `solve_2x2_mixed_nash()` from `R/solvers.R` implements @eq-q-star and @eq-p-star directly.

```{r}
#| label: matching-pennies-solver
...
#| fig-cap: "Player 1's expected payoff from each pure action as a function of Player 2's mixing probability $q$ in Matching Pennies. The lines cross at $q^* = 0.5$, the indifference point where Player 1 is willing to randomize."
#| fig-width: 6
#| fig-height: 4.5
...
- **Extensive-form games** (@sec-extensive-form) use behavioral strategies --- independent mixtures at each information set --- which are equivalent to mixed strategies under perfect recall.
- In **Bayesian games** (@sec-bayesian-games), a player's type can be viewed as a private signal that induces mixing from the opponent's perspective, blurring the line between mixed and pure strategies.
