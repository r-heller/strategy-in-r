...
```{r, run-regret-matching}
set.seed(42)
rm_result <- regret_matching(n_iterations = 5000)
```

```{r, regret-convergence, fig.cap="Regret matching convergence in Rock-Paper-Scissors. Player 1's time-averaged strategy converges toward the Nash equilibrium of (1/3, 1/3, 1/3) for each action.", fig.width=7, fig.height=5}
...
```{r, decision-boundary, fig.cap="Decision boundary for predicting cooperation. The logistic regression separates cooperators (orange) from defectors (blue) based on the opponent's historical cooperation rate and normalized round number.", fig.width=7, fig.height=5}
