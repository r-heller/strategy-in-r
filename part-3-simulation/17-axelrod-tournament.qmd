...
author: "Raban Heller"
date-modified: today
abstract: "A computational replication of Axelrod's 1980 iterated Prisoner's Dilemma tournament in R, exploring why Tit for Tat won and what it teaches about the evolution of cooperation."
keywords: ["Axelrod", "tournament", "Prisoner's Dilemma", "Tit for Tat", "cooperation", "iterated games"]
other-links:
  - text: Download Chapter PDF
    href: ../downloads/replicating-axelrods-tournament.pdf
    icon: file-pdf
---
...
```{r}
#| label: fig-axelrod-ranking
#| fig-cap: "Tournament ranking by total score. Tit for Tat wins the round-robin tournament, consistent with Axelrod's original finding. Nice strategies (those that never defect first) cluster at the top."
#| fig-width: 7
#| fig-height: 5
...
```{r}
#| label: fig-axelrod-heatmap
#| fig-cap: "Head-to-head scores in the round-robin tournament. Rows are the focal player; columns are the opponent. Always Defect scores highest against Always Cooperate but poorly against retaliatory strategies."
#| fig-width: 7
#| fig-height: 6
...
- **Evolutionary dynamics**: What happens when successful strategies reproduce and unsuccessful ones die out? This leads to replicator dynamics on strategy populations (@sec-replicator-dynamics) and the concept of evolutionarily stable strategies (@sec-ess).
- **Spatial structure**: When players interact only with neighbors on a lattice, cooperation can survive even without reciprocity (@sec-spatial-pd).
...
- For the original analysis, see @axelrod1981. For a modern computational treatment with hundreds of strategies, see the Python `axelrod` library (accessible via `reticulate`; see @sec-nashpy-reticulate for the general pattern).
